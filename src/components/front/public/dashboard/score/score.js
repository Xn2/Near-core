...
async function refreshTable() {
  ...
  try {
    ...
    if (
      diff === 3 &&
      songInfo.difficulties[songInfo.difficulties.length - 1].diff === 'MAXIMUM'
    ) {
    ...
    document.getElementById(
      'nameTitle'
    ).innerText = `${songInfo.title} - ${songInfo.difficulties[diff].diff} ${songInfo.difficulties[diff].level}`;
    let img1 = document.createElement('img');
    img1.setAttribute(
      'src',
      `https://fairyjoke.net/api/games/sdvx/musics/${id}/${
        songInfo.difficulties[diff - 1].diff
