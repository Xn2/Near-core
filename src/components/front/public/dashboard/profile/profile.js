  ...
  for (obj of arr) {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      td.addEventListener('click', () => {
        window.location.href = '/web/dashboard/score?m=' + id;
      });
      tr.appendChild(td);
    }
    let td = document.createElement('td');
...
async function refreshTable() {
  ...
  apeca.setAttribute(
    'src',
    'https://fairyjoke.net/api/games/sdvx/apecas/' + profile.apecaID + '.png'
  );
  ...
  document.getElementById('joined').innerText = getFormattedDate(
    profile.createdAt
  );
  document.getElementById('lastOnline').innerText = getFormattedDate(
    profile.updatedAt
  );
