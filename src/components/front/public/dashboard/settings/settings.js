document.addEventListener('DOMContentLoaded', async function (e) {
  document
    .getElementById('changePasscode')
    .addEventListener('click', function () {
      changePasscode();
    });
  document
    .getElementById('changePlayerName')
    .addEventListener('click', function () {
      changePlayerName();
    });
  document
  ...
  const settings = await (
    await fetch('/web/dashboard/settings/settings.json')
  ).json();
  ...
  for (akaname of settings.akaname) {
    ...
    if (akaname.value == currentSettings.akaname)
  ...
  for (nemsys of settings.nemsys) {
    ...
    if (nemsys.value == currentSettings.nemsys)
      el.setAttribute('selected', true);
  ...
  for (stamp of settings.stamp) {
    for (let i = 0; i < 4; i++) {
      ...
      if (stamp.value == currentSettings['stamp' + String.fromCharCode(65 + i)])
        el.setAttribute('selected', true);
      document
        .getElementById('stamp' + String.fromCharCode(65 + i))
        .appendChild(el);
    }
  }
});
...
async function changePasscode() {
  ...
  const previousPassCode = parseInt(
    document.getElementById('previousPassCode').value
  );
  const res = await fetch('/api/me/changePasscode', {
    method: 'post',
    headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
    body: JSON.stringify({ passCode, previousPassCode }),
  });
  if (res.status == 200) {
...
async function changePlayerName() {
  ...
  const res = await fetch('/api/me/changePlayerName', {
    method: 'post',
    headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
...
async function setSV6Settings() {
  ...
  const res = await fetch('/api/me/setSV6Settings', {
    method: 'post',
    headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
    body: JSON.stringify({
      akaname,
