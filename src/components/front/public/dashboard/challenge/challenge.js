...
async function refreshTable() {
  ...
  try {
    ...
    for (course of courses) {
      if (course.id == id) {
        ...
        img1.setAttribute(
          'src',
          `https://fairyjoke.net/api/games/sdvx/musics/${course.tracks[0].mid}/NOVICE.png?fallback=default`
        );
        img2.setAttribute(
          'src',
          `https://fairyjoke.net/api/games/sdvx/musics/${course.tracks[1].mid}/NOVICE.png?fallback=default`
        );
        img3.setAttribute(
          'src',
          `https://fairyjoke.net/api/games/sdvx/musics/${course.tracks[2].mid}/NOVICE.png?fallback=default`
        );
  ...
  document
    .getElementById('recentScores')
    .appendChild(await createTable(scores.slice(0, 20), 'scores'));
  console.log(courses);
  console.log(scores);
}
